...
def main() -> int:
    ...
    p.add_argument(
        "--requests-dir",
        default=None,
        help="directory of recorded requests (default requests/ = finance run recorded on the broken "
        "build; requests-concept/ = concept run recorded on the fixed build; requests-killed/ = the "
        "four generations killed under the benchmark)",
    )
    p.add_argument(
        "--timeout",
        type=float,
    ...
    recorded_model, requests = load_requests(Path(args.requests_dir) if args.requests_dir else None)
